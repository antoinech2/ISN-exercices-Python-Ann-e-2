...
"Programme qui affiche un dessin comportant des carrés, triangles, étoiles à 5 et 6 branches."
...
def __main__(): #Définition de la fonction principale du programme
    ...
    size = 50 #On définit la taille initiale des figures (50 pixels)
    ...
    for loop in range (9): #On définit la suite de figures qui est répétée 9 fois
        ...
        DrawStar6(size) #On trace une étoile à 6 branches.
        #C'est cette fonction qui applique l'angle qui fait "tourner" la figure
        fd(size+7)
        DrawTriangle(size) #On trace un triangle
        fd(size+7)
        DrawStar5(size)  #Enfin, on trace l'étoile à 5 branches
        fd(size+7)
        size-=4   #On réduit la taille des figures de 4 pixel pour la prochaine suite d'instructions

    hideturtle() #On masque la tortue à la fin du dessin pour que le résultat corresponde à la consigne
    exitonclick()   #On appelle cette fonction pour que la fenêtre se ferme lors du clic de l'utilisateur
                    #(permet à la fenêtre de rester ouverte).

__main__() #On exécute la fonction principale
