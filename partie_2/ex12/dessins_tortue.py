...
"Programme qui contient les fonctions pour dessiner des formes prédéfinies"
"(carré, triangle, étoile à 5 et 6 branches)."
...
def DrawSquare(size):
    "Dessine un carré à la position de la tortue"
    down()
...
def DrawTriangle(size):
    "Dessine un triangle à la position de la tortue"
    down()
...
# Paramètres: taille (longeur du côté) (integer)
def DrawStar5(size):
    "Dessine une étoile à 5 branches à la position de la tortue"
    down()
...
def DrawStar6(size):        #L'étoile est composée de 6 triangles équilatéraux de sens opposés.
    "Dessine une étoile à 6 branches à la position de la tortue"
    ...
    left(120)               #On s'oriente vers le sommet de l'autre triangle (direction de la turtle
                            #perpendiculaire au côté du triangle actuel)
    ...
    #Note: l'orientation de la turtle est différente entre le premier et le deuxième triangle,
    #ce qui applique l'angle sur la totalité de la figure.
    up()
